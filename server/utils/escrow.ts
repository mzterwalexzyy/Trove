...
import { bounties, escrowLog, payouts, transactions, useDb } from '../db'
import {
  addressesMatch,
  decodeMemo,
  fetchTransaction,
...
/**
 * Looks for a payout we may have already broadcast for this bounty. Used to
 * recover from a crash between broadcasting and recording the hash, so a
 * retry adopts the existing transaction instead of paying twice.
 */
async function findExistingPayoutOnChain(bountyId: string, escrowAddress: string): Promise<string | null> {
  const memo = payoutMemo(bountyId)
  try {
...
export async function payWinner(bountyId: string): Promise<PayoutResult> {
  ...
  // The reward can never exceed what the chain confirmed arrived.
  const amountLuna = Math.min(bounty.rewardLuna, bounty.fundedLuna)
  if (amountLuna <= 0) return { ok: false, reason: 'nothing to pay' }
  ...
  try {
    ...
    const already = await findExistingPayoutOnChain(bountyId, escrowAddress)
    if (already) {
      await db.update(payouts)
