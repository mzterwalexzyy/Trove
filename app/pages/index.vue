...
/**
 * Inside Nimiq Pay this page is a detour. The deeplink can only carry a bare
 * domain, so the wallet always arrives at `/`; bouncing to the app here is
 * what makes `/app` reachable from a deeplink at all.
 */
onMounted(() => {
    ...
    <div class="relative">
      ...
      <div class="mx-auto flex w-full max-w-[1600px] items-center justify-between px-6 py-5 lg:px-12 lg:py-6">
        ...
        <div class="flex items-center gap-2">
          <ThemeToggle />
          <!-- The deeplink sits in the header now. It is the route for people
               who already have the wallet, which is the smaller group. -->
          <a
            :href="nimiqPayLink"
            class="pressable flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2.5 text-[13px] font-semibold text-ink lg:px-5"
          >
            <svg viewBox="0 0 24 24" fill="currentColor" class="size-4 text-brand">
            ...
            <span class="hidden sm:inline">Open in Nimiq Pay</span>
            <span class="sm:hidden">Nimiq Pay</span>
          </a>
        </div>
      </div>
    </header>
      ...
      <div class="w-full max-w-md text-center lg:max-w-[46rem] lg:pl-14 lg:text-left">
        ...
        >
          <!-- Nimiq Pay leads. This is a Mini App: the wallet is where it
               ...
               way round. -->
          <!-- Launch is the primary action: it works for everyone, wallet or
               not, where the deeplink only works if Nimiq Pay is installed. -->
          <button
            class="pressable flex min-h-[52px] items-center justify-center gap-2 rounded-2xl bg-brand px-7 text-[15px] font-semibold text-white disabled:opacity-60"
      ...
      >
        <div class="lg:flex lg:items-center lg:justify-between lg:gap-10">
          ...
          <div class="mt-7 flex shrink-0 flex-col gap-2.5 sm:flex-row sm:justify-center lg:mt-0">
            <a
              :href="nimiqPayLink"
              class="pressable flex min-h-[52px] items-center justify-center gap-2.5 rounded-2xl bg-brand px-7 text-[14.5px] font-semibold text-white"
            >
